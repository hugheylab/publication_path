...
def get_app_info():
    return(get_orcid_app_info())

def get_oauth_link(scope, app_key):
    if app_key is None:
        db = get_db()
        app_key = get_app_info(db)
        db.close()
    orcid_auth_link = oauth_base_url + '?client_id=' + app_key['client_id'] + '&response_type=code&scope=' + scope + '&redirect_uri=' + redirect_uri
